...
static void collect_pt_nodemask(struct vm_area_struct *vma, unsigned long addr, nodemask_t *nodemask) {
	struct mm_struct *mm = vma->vm_mm;
	pgd_t *pgd; p4d_t *p4d; pud_t *pud; pmd_t *pmd;
	struct page *ptpage, *current_page;
	...
	current_page = ptpage = pmd_page(*pmd);
...
static void flush_tlb_vma_mm_range(struct mm_struct *mm, struct vm_area_struct *vma, unsigned long start,
				unsigned long end, unsigned long vmflag)
{
	int cpu, node;
	cpumask_t flush_mask = {0};
	nodemask_t flush_node_mask = {0};
	...
	if (mm->lazy_repl_enabled && sysctl_hydra_tlbflush_opt && vma && ((start >> PMD_SHIFT) == (end >> PMD_SHIFT))) {
		collect_pt_nodemask(vma, start, &flush_node_mask);
		for_each_node_mask(node, flush_node_mask) {
			cpumask_or(&flush_mask, &flush_mask, cpumask_of_node(node));
		}
		cpumask_and(&flush_mask, &flush_mask, mm_cpumask(mm));
	} else {
		cpumask_copy(&flush_mask, mm_cpumask(mm));
	}
...
void flush_tlb_mm_range(struct mm_struct *mm, unsigned long start,
				unsigned long end, unsigned long vmflag) {
	flush_tlb_vma_mm_range(mm, NULL, start, end, vmflag);
}

void flush_tlb_vma_range(struct vm_area_struct *vma, unsigned long start,
				unsigned long end, unsigned long vmflag) {
	flush_tlb_vma_mm_range(vma->vm_mm, vma, start, end, vmflag);
}
